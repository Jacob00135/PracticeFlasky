...
;(function(window, document) {
  ...
  // var markdownEl = document.querySelector('textarea.markdown-edit'),
  ...
  // // newNode.id = 'content';
  // document.querySelector('.markdown-edit-containter').replaceChild(newNode, markdownEl);
  ...
  window.MyGlobalFunction.marked = function (container) {
    /**
     * @function 渲染markdown文本
     * @param container {Node} markdown容器，子元素必须要有markdown编辑器节点、预览框节点
     */

    // 获取编辑器节点、预览框节点
    const edit = container.querySelector('.markdown-edit');
    const preview = container.querySelector('.markdown-preview');

    // 使用新预览节点替代旧预览节点，展现效果为渲染markdown文本
    var newNode = document.createElement('div');
    newNode.setAttribute('class', 'container-fluid markdown-preview');
    container.replaceChild(newNode, preview);
    newNode.innerHTML = marked(edit.value || edit.textContent || edit.innerText);
